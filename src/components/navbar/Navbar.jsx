...
const Navbar = () => {
  const navigate = useNavigate();
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);
  const [openMobileSubmenu, setOpenMobileSubmenu] = useState(null); // মোবাইল একর্ডিয়ন স্টেটের জন্য
  const { totalItems, setIsCartOpen } = useCart();
  ...
  const menuCategories = [
    ...
    { 
      name: "Accessories", 
      subCategories: ["Shoes","Singles", "Jewelry", "Bags"] 
    },
    { 
  ...
  const handleCategoryClick = (categoryName, subCategoryName = null) => {
    ...
    let url = `/collection?category=${encodeURIComponent(categoryName)}`;
    if (subCategoryName) {
  ...
  return (
    <>
      ...
      >
        ...
        {/* ================= বটম মেনু (ডেস্কটপ উইথ সাব-ক্যাটাগরি ড্রপডাউন) ================= */}
        <div className={`hidden md:block border-gray-50 bg-white transition-all duration-300 ease-in-out ${
          isMiniNavbar ? "py-2.5 shadow-sm" : "py-4"
        }`}>
          <div className="max-w-7xl mx-auto px-4">
            <div className="flex flex-wrap justify-center items-center gap-x-8 lg:gap-x-10 gap-y-2">
              {menuCategories?.map((item, index) => (
                ...
                >
                  <button
                    ...
                    className="opacity-0 translate-y-3 h-[25px] animate-navLink text-[13px] lg:text-[14px] font-semibold uppercase tracking-[0.18em] text-neutral-800 hover:text-[#b5832a] transition-colors relative duration-300 whitespace-nowrap cursor-pointer flex items-center gap-1"
                  >
                    {item.name}
                    {item.subCategories && (
                      <ChevronDown size={14} className="transition-transform duration-300 group-hover:rotate-180 text-gray-500" />
                    )}
                    <span className="absolute bottom-[-5px] left-1/2 w-0 h-[1.5px] bg-[#b5832a] transition-all group-hover:w-full group-hover:left-0"></span>
                  </button>

                  {/* 🌟 ডেস্কটপ সাব-ক্যাটাগরি ড্রপডাউন প্যানেল */}
                  {item.subCategories && (
                    <div className="absolute left-0 top-full pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 transform group-hover:translate-y-0 translate-y-2 z-50">
                      <div className="w-48 bg-white border border-neutral-100 shadow-xl rounded-md py-2 px-1">
                        {item.subCategories.map((subCat) => (
                          <button
                            ...
                            className="block w-full text-left px-4 py-2 text-[12px] uppercase tracking-wider text-neutral-600 hover:text-[#b5832a] hover:bg-neutral-50/80 rounded transition-colors cursor-pointer"
                          >
                            {subCat}
                          </button>
                        ))}
      ...
      >
        ...
        {/* 📱 মোবাইল ক্যাটাগরি ও সাব-ক্যাটাগরি লিস্ট (Accordion Style) */}
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-1">
          {menuCategories?.map((item) => (
            <div key={item.name} className="border-b border-neutral-100/60 last:border-0">
              <div className="flex justify-between items-center">
                <button
                  onClick={() => handleCategoryClick(item.name)}
                  className="text-left text-[13px] font-medium tracking-widest uppercase text-neutral-700 hover:text-[#b5832a] py-3 transition-all cursor-pointer flex-1"
                >
                  {item.name}
                </button>
              ...
              {/* মোবাইল ড্রপডাউন সাব-ক্যাটাগরি কন্টেন্ট */}
              {item.subCategories && openMobileSubmenu === item.name && (
                <div className="pl-4 pb-2 space-y-1.5 bg-neutral-50/50 rounded-md my-1">
                  {item.subCategories.map((subCat) => (
                    <button
                      key={subCat}
                      onClick={() => handleCategoryClick(item.name, subCat)}
                      className="block w-full text-left text-[11px] font-medium tracking-wider uppercase text-neutral-600 hover:text-[#b5832a] py-1.5 cursor-pointer"
                    >
                      • {subCat}
                    </button>
                  ))}
